...
function makeWord(obj) {
  const arrayWithNums = Object.values(obj).flat();
  const newArr = new Array(arrayWithNums.length);
...
function sellTickets(queue) {
  const firstNot25 = queue.findIndex((mon) => mon !== 25);
  const sumBefore = queue.slice(0, firstNot25);
  if (sumBefore < queue[firstNot25]) return false;
  return true;
}
...
function sortCitiesArray(arr) {
  ...
  const sortArr = newArr.sort((a, b) =>
    a.country === b.country
      ? sortByLetters('city')(a, b)
      : sortByLetters('country')(a, b)
  );
  return sortArr;
}
